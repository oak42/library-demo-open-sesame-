<%--@elvariable id="fnlReviewList" type="com.ackerley.library.modules.priorBookCircu.entity.BulkAuditingSFAid"--%>
<html>
<head>
    <title>建议终审(二审)</title>
...
<div id="overallAccordion">
    <div id="listHeaderPlaceholder" class="row no-gutters">
        <div id="listHeaderFixedTop" class="col-12 fixed-top" style="background-color: #c82333;">
            <div class="row no-gutters listHeaderRow">
                <div class="col-8 listHeader">--终审--</div>
    ...
    <c:if test="${fnlReviewList.list.size() == 0}">
        <sys:message content="已无更多待终审项..." type="warning"/>
    </c:if>
    <c:if test="${fnlReviewList.list.size() > 0}">
        <sf:form modelAttribute="fnlReviewList" method="post" action="${ctx}/priorCircu/fnlRvw/submit">

            <c:forEach var="index" begin="0" end="${fnlReviewList.list.size() - 1}"><!--【扣】待试：若使用items而非begin、end来用index自己为path赋名，是否就能用上泛型版本的入参绑定了？-->
                            ...
                            <div class="col-7 ">
                                <button class="btn btn-outline-light collapseTrigger" type="button" data-toggle="collapse" data-target="#collapse-${index}" title="点击查看详细(参考豆瓣)...">
                                    ...
                                    <label class="bookMajorDetailLabel" for="ISBN13-${index}">ISBN13：
                                        <sf:input id="ISBN13-${index}" class="ISBN13" size="13" path="list[${index}].subject.ISBN13"/>
                                    </label>
                                    <label class="bookMajorDetailLabel" for="title-${index}">title：
                                        <sf:input id="title-${index}" path="list[${index}].subject.title"/>
                                    </label>
                                </button>
                            </div>
